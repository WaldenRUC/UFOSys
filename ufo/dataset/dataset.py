import os, json, random, warnings, datasets, rich
...
class Item:
    ...
    def __init__(self, item_dict: Dict[str, Any]) -> None:
        ...
        self.response: Optional[str] = item_dict.get('response', None)
        self.reference_answers: List[str] = item_dict.get("reference_answers", [])
        self.label: Optional[float] = item_dict.get("label", None)
        self.output: List[str, Any] = item_dict.get("output", [])
        item_dict['output_metric'] = {}
        self.output_metric: Dict[str, Any] = item_dict.get("output_metric", {})
    ...
    def update_output(self, key: str, value: Any) -> None:
        ...
        if key in ["id", "question", "reference_answers", 'label', "output"]:
            raise AttributeError(f"{key} in output should not be changed")
        else:
            if isinstance(value, list):    
                if self.output == []:
                    self.output = [{} for _ in value]
                for _id, _value in enumerate(value):
                    self.output[_id][key] = _value
            else:
                for _id in range(len(self.output)):
                    self.output[_id][key] = value
    def update_evaluation_score(self, metric_name: str, metric_score: float) -> None:
        ...
        self.output_metric[metric_name] = metric_score
        
    def __getattr__(self, attr_name: str) -> Any:
        predefined_attrs = ["id", "question", "reference_answers", "output"]
    ...
    def __setattr__(self, attr_name: str, value: Any) -> None:
        predefined_attrs = ["id", "question", "reference_answers", 'response', "output", 'data', 'label']
        if attr_name in predefined_attrs:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        from ufo.dataset.utils import convert_numpy, remove_images
...
class Dataset:
    ...
    def __init__(
        ...
        config = None,
        dataset_path = None,
        data: Optional[List[Dict[str, Any]]] = None,    # 列表元素为dict或者item
        sample_num = None,
        ...
        if data is None:    # 未给定data -> 从文件中加载数据
    ...
    def _load_data(self, dataset_name: str, dataset_path: str) -> List[Item]:
        """Load data from the provided dataset_path"""
    ...
    def update_output(self, key: str, value_list: List[Any]) -> None:
        """用value_list更新所有item的key对应的值"""
        assert len(self.data) == len(value_list), f'{len(self.data)}; {len(value_list)}'
    ...
    @property
    def reference_answers(self) -> List[List[str]]:
        return [item.reference_answers for item in self.data]
